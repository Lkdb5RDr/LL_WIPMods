...
RelocAddr <_InstallArmorAddon> InstallArmorAddon_Original(0x0035ECF0);	// also ADF4::ID2194390
RelocAddr <uintptr_t> InstallArmorAddon_Start(0x00359870 + 0xB4C);	// ADF4::ID2194348

typedef void (* _ApplyMaterialProperties)(NiAVObject * object);
RelocAddr <_ApplyMaterialProperties> ApplyMaterialProperties(0x021C1190); // 42D562E443C0313BACEF58FC1A4508489CED355F+33A also BSShaderutil::ClearRenderPasses or ADF4::ID2317600
RelocAddr <uintptr_t> HairColorModify_Start(0x006EE750 + 0x262);	// also BSFaceGenUtils::PrepareHeadPartForShaders or ADF4::ID2209538

RelocAddr <uintptr_t> GetHairTexturePath_Start(0x006EBD90 + 0x10d4);	// also BSFaceGenUtils::StartFaceCustomizationGenerationForNPC or ADF4::ID2209525 

RelocPtr<UInt32> g_faceGenTextureWidth(0x02F42910); // F5F0D2A6AFBE88D06472E751C88521770B465B79+148	also ADF4::ID2666739
...
RelocAddr <_InitializeSharedTarget> InitializeSharedTarget(0x01838B50);	// also RenderTargetManager::CreateRenderTarget or ADF4::ID2277176
...
void InitializeSharedTarget_Hook(BSGraphics::RenderTargetManager * targetManager, UInt32 type, BSGraphics::RenderTargetManager::RenderTargetProperties* targetProperties, UInt8 unk1)
{
	switch(type)
	{
	case 16:
	case 17:
	case 18:
		targetProperties->uiWidth = g_tintMaskWidth;
		targetProperties->uiHeight = g_tintMaskHeight;
		break;
	}
	InitializeSharedTarget_Original(targetManager, type, targetProperties, unk1);
}
...
{
	if (kPluginHandle_Invalid == g_pluginHandle)
	{
		PluginInfo info;
		if (!F4SEPlugin_Query(skse, &info)) return false;
	}
	...
	if(g_bEnableBodyMorphs || g_bEnableOverlays)
	{
		struct InstallArmorAddon_Code : Xbyak::CodeGenerator {
			InstallArmorAddon_Code(void * buf, UInt64 funcAddr) : Xbyak::CodeGenerator(4096, buf)
			{
				...
				mov(r8d, ptr[rbp+0x1C0-0x210]); // Stack var to current index of equip item loop // changed from 0x208
				mov(rdx, r12);					// ObjectNode // same in 1.11.169
				mov(rcx, ptr[rsp+0x2A8-0x268]); // Actor - rbx - ptr[rsp+0x2A8-0x268] // same in 1.11.169
	...
	// Resizes tint mask target
	{
		struct InitializeSharedTarget_Code : Xbyak::CodeGenerator {
			InitializeSharedTarget_Code(void * buf) : Xbyak::CodeGenerator(4096, buf)
			{
				...
				push(rdi);	// changed from 1.10.163
				push(r14);
				push(r15);
				...
				L(retnLabel);
				dq(InitializeSharedTarget.GetUIntPtr() + 6);	// Changed from 1.10.163
			}
		};
