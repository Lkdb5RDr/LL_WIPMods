...
void F4EEBodyGenUpdate::Run()
{
	...
	if(form) {
		...
		if(actor) {
				...
				if(m_doDetach)
				{
					const BSTSmartPointer<BipedAnim>& GetBipedAnim(bool bFirstPerson);
				...
				if(middleProcess) 
					middleProcess->DoUpdate3dModel(actor, 0x11);
#ifdef _DEBUG_MOPRHING
				else
...
bool BodyMorphInterface::ApplyMorphsToShape(Actor * actor, const MorphableShapePtr & morphableShape)
{
	...
	if(geometry) {
		...
		void * baseData = geometry->pRendererData;
		void * geomData = nullptr;
		if(!baseData)
			return false;

		auto vertexData = static_cast<UInt8*>(static_cast<BSGraphics::TriShape*>(baseData)->pVB->pData);
		if(!vertexData)
			return false;
		...
		geomData = g_renderManager->CreateTriShape(&blockSize, vertexData, geometry->vertexDesc, static_cast<BSGraphics::TriShape*>(baseData)->pIB);
		if(!geomData)
			return false;

		newBlock = static_cast<UInt8*>(static_cast<BSGraphics::TriShape*>(geomData)->pVB->pData);
