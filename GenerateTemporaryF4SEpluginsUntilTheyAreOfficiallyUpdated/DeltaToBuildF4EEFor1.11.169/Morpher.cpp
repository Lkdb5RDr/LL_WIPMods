...
MorphApplicator::MorphApplicator(BSTriShape * _geometry, UInt8 * srcBlock, UInt8 * dstBlock, std::function<void(std::vector<Morpher::Vector3> &)> morph) : geometry(_geometry), morphFunc(morph)
{
	...
	void * geomData = geometry->pRendererData;
	...
	if (geomData)
	{
		...
		UInt8* vertexBlock = srcBlock ? srcBlock : static_cast<UInt8*>(static_cast<BSGraphics::TriShape*>(geomData)->pVB->pData); // geomData->vertexData->vertexBlock;
		...
		auto triangleData = static_cast<UInt16*>(static_cast<BSGraphics::TriShape*>(geomData)->pIB->pData);
		if (triangleData)
			triangles = (Morpher::Triangle*)triangleData;

		RecalcNormals(geometry->numTriangles, triangles);
		CalcTangentSpace(geometry->numTriangles, triangles);

		vertexBlock = dstBlock ? dstBlock : static_cast<UInt8*>(static_cast<BSGraphics::TriShape*>(geomData)->pVB->pData); // geomData->vertexData->vertexBlock;
		for (UInt32 i = 0; i < numVertices; i++)
		{
			UInt8* vBegin = &vertexBlock[i * vertexSize];
